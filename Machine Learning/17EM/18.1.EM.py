...
if __name__ == '__main__':
    ...
    if style == 'sklearn':
        g = GaussianMixture(n_components=2, covariance_type='full', tol=1e-6, max_iter=1000)#c_type 方差类别 有  spherical，diga,tied,full
        g.fit(data)
        ...
        for i in range(num_iter):
            ...
            # M Step
            mu1 = np.dot(gamma, data) / np.sum(gamma)#.dot內积
    ...
    order = pairwise_distances_argmin([mu1_fact, mu2_fact], [mu1, mu2], metric='euclidean')#类标号要对应上
    print(order)
    if order[0] == 0:
        c1 = tau1 > tau2
    else:
