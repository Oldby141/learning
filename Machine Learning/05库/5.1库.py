...
# (-x) ** (-x)  x < 0
def f(x):
    y = np.ones_like(x)#生成一个元素为1的相同矩阵
    i = x > 0
...
# # 3.2.2
    ...
    a = np.random.rand(10)#Random values in a given shape.
...
# # 合并上述代码：
    ...
    # print (a[(0,1,2,3), (2,3,4,5)])
    # print (a[3:, (0, 2, 5)])#print a[3:, [0, 2, 5]]也可以
    # i = np.array([True, False, True, False, False, True])
    # print (a[i])
    # print (a[i, 3])
# # 4. numpy与Python数学库的时间比较
#     for j in np.logspace(0, 7, 10):#numpy 更快
#         j = int(j)
...
# 5.绘图
    ...
    # plt.plot(x, y, 'r-', x, y, 'go', linewidth=2, markersize=12)
    # #plt.plot(x, y, 'r-', x, y, 'go', linewidth=4, markersize=8)
    # plt.grid(True)#网格
    # plt.show()
    #
...
#     plt.grid()
#     plt.legend(loc='upper right')#把它们的图例标签放在一起 位置右侧
#     # plt.savefig('1.png')
#     plt.show()
    # # 5.3 x^x
...
#     plt.show()
    ...
    # t = np.arange(len(x))
    # plt.hist(x, 30, color='m', alpha=0.5)#30指有30条条状图
