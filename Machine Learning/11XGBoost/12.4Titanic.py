...
def load_data(file_name,is_train):
    ...
    if len(data.Fare[data.Fare.isnull()]) >0:
        fare = np.zeros(3)
        for f in range(0,3):
            fare[f] = data[data.Pclass == f + 1]['Fare'].dropna().median()#。dropna
        for f in range(0,3):
            data.loc[(data.Fare.isnull())&(data.Pclass ==  f+1),'Fare'] = fare[f]#loc可以让你按照索引来进行行列选择
    ...
    # print( data['Embarked'])
    embarked_data = pd.get_dummies(data.Embarked)#完成独热编码  只有三列
    # print (embarked_data)
    # embarked_data = embarked_data.rename(columns={'S': 'Southampton', 'C': 'Cherbourg', 'Q': 'Queenstown', 'U': 'UnknownCity'})
    embarked_data = embarked_data.rename(columns=lambda x: 'Embarked_' + str(x))
    data = pd.concat([data, embarked_data], axis=1)#concat函数是在pandas底下的方法，可以将数据根据不同的轴作简单的融合
    print(data.describe())
